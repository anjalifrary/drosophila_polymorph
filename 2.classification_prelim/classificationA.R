...
rds_file <- paste0("/scratch/ejy4bu/drosophila/gds_analysis/snp_datatables/test_files/shared_dt_chr2L.rds")
...
shared_classA <- shared_dt[
    !is.na(shared_dt$af_mel) & 
    !is.na(shared_dt$af_sim) & 
    !is.na(shared_dt$aa_sub_mel) &
    !is.na(shared_dt$aa_sub_sim) &
    shared_dt$aa_sub_mel == shared_dt$aa_sub_sim
]
...
if(nrow(shared_classA) < 600){
    ...
    subset_table <- shared_classA[1:500, ]
...
### get a table with all the rows where the same pos and same nt 
out_dir <- "/scratch/ejy4bu/drosophila/gds_analysis/snp_datatables/test_files/"
if(!dir.exists(out_dir)) dir.create(out_dir)
out_csv <- paste0(out_dir, "all_shared_variants_chr2L.csv")
out_rds <- paste0(out_dir, "all_shared_variants_chr2L.rds")
if(!file.exists(out_csv)) file.create(out_csv)
if(!file.exists(out_rds)) file.create(out_rds)
...
message(nrow(shared), " shared variants with same pos & nt")
saveRDS(shared, out_rds)
message("RDS saved to ", out_rds)

if(nrow(shared) < 600){
    fwrite(shared, out_csv)
    message("CSV saved to ", out_csv)
} else {
    subset_table <- shared[1:500, ]
    fwrite(subset_table, out_csv)
    message("saved first 500 rows to csv at ", out_csv)
}
