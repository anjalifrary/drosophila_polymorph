...
# # background file, containing all species-specific variants that passed QC filters and 5% MAF filters

# this isn't working... 

merged_dt <- readRDS("/scratch/ejy4bu/drosophila/gds_analysis/snp_dt_analysis/merged_tables/quality/all_quality_variants_merge_unfilt.rds")
nrow(merged_dt[!is.na(ref_mel)])
nrow(merged_dt[!is.na(ref_sim)])

nrow(mel_nlp) # discrepancies in numbers bc only kept nlp > 0 variants; close enough
nrow(sim_nlp)

mel_maf <- mel_nlp[poly_af>0.05 & poly_af<0.95, .(chr, pos)]
mel_maf[, keep_mel := TRUE]
nrow(mel_maf)

sim_maf <- sim_nlp[poly_af>0.05 & poly_af<0.95, .(chr, pos)]
sim_maf[, keep_sim := TRUE]
nrow(sim_maf)

merged_dt <- merge(merged_dt, mel_maf, by = c("chr", "pos"), all.x = TRUE)
merged_dt <- merge(merged_dt, sim_maf, by = c("chr", "pos"), all.x = TRUE)

merged_dt[is.na(keep_mel), keep_mel := FALSE]
merged_dt[is.na(keep_sim), keep_sim := FALSE]

background <- merged_dt[keep_mel == TRUE | keep_sim == TRUE]

nrow(background[!is.na(ref_mel)])
nrow(background[!is.na(ref_sim)])
...
background <- background[
  ,
  .(
    keep_mel = any(keep_mel, na.rm = TRUE),
    keep_sim = any(keep_sim, na.rm = TRUE)
  ),
  by = .(chr, pos)
]
...
background[is.na(keep_mel), keep_mel := FALSE]
background[is.na(keep_sim), keep_sim := FALSE]

setkey(merged_dt, chr, pos)
setkey(mel_maf, chr, pos)
setkey(sim_maf, chr, pos)

merged_dt <- mel_maf[merged_dt]
merged_dt <- sim_maf[merged_dt]

merged_dt[is.na(keep_mel), keep_mel := FALSE]
merged_dt[is.na(keep_sim), keep_sim := FALSE]

background <- merged_dt[keep_mel | keep_sim]